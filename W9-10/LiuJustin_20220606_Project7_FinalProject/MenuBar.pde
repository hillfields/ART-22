class MenuBar {
  PImage battery, wifi;                                            // icons on top right of bar
  ArrayList<MenuButton> menuButtons = new ArrayList<MenuButton>(); // to store buttons

  // constructor
  MenuBar() {
    textAlign(LEFT, TOP); // reference text by left and top coordinates
  ...
  void display() {
    ...
    image(battery, 1075, 2.5); // battery icon
    image(wifi, 1120, 5);      // wifi icon
